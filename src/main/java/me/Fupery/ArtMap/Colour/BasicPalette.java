...
public class BasicPalette implements Palette {
	...
	VOID = new BasicDye(DYE_VOID.get(),"VOID", 0, ChatColor.DARK_GREEN, Material.EYE_OF_ENDER),
			GRASS = new BasicDye(DYE_GRASS.get(), "GRASS", 1, ChatColor.DARK_GREEN, Material.SEEDS),
			CREAM = new BasicDye(DYE_CREAM.get(), "CREAM", 2, ChatColor.GOLD, Material.PUMPKIN_SEEDS),
			LIGHT_GRAY = new BasicDye(Lang.DYE_LIGHT_GRAY.get(), "LIGHT_GRAY", 3, ChatColor.GRAY, Material.WEB), // new
			RED = new BasicDye(DYE_RED.get(), "RED", 4, ChatColor.RED, Material.INK_SACK, 1),
			ICE = new BasicDye(Lang.DYE_ICE.get(), "ICE", 5, ChatColor.GRAY, Material.ICE), // new
			SILVER = new BasicDye(DYE_SILVER.get(), "SILVER", 6, ChatColor.GRAY, Material.INK_SACK, 7),
			LEAVES = new BasicDye(Lang.DYE_LEAVES.get(), "LEAVES", 7, ChatColor.GREEN, Material.LEAVES), // new
			SNOW = new BasicDye(Lang.DYE_SNOW.get(), "SNOW", 8, ChatColor.BLUE, Material.SNOW), // new
			GRAY = new BasicDye(DYE_GRAY.get(), "GRAY", 9, ChatColor.DARK_GRAY, Material.INK_SACK, 8),
			...
			DARK_WOOD = new BasicDye(Lang.DYE_DARK_WOOD.get(), "DARK_WOOD", 13, ChatColor.GREEN, Material.LOG_2, 1), // new
			WHITE = new BasicDye(DYE_WHITE.get(), "WHITE", 14, ChatColor.WHITE, Material.INK_SACK, 15),
			ORANGE = new BasicDye(DYE_ORANGE.get(), "ORANGE", 15, ChatColor.GOLD, Material.INK_SACK, 14),
			MAGENTA = new BasicDye(DYE_MAGENTA.get(), "MAGENTA", 16, ChatColor.LIGHT_PURPLE, Material.INK_SACK, 13),
			LIGHT_BLUE = new BasicDye(DYE_LIGHT_BLUE.get(), "LIGHT_BLUE", 17, ChatColor.BLUE, Material.INK_SACK, 12),
			YELLOW = new BasicDye(DYE_YELLOW.get(), "YELLOW", 18, ChatColor.YELLOW, Material.INK_SACK, 11),
			LIME = new BasicDye(DYE_LIME.get(), "LIME", 19, ChatColor.GREEN, Material.INK_SACK, 10),
			PINK = new BasicDye(DYE_PINK.get(), "PINK", 20, ChatColor.LIGHT_PURPLE, Material.INK_SACK, 9),
			GRAPHITE = new BasicDye(DYE_GRAPHITE.get(), "GRAPHITE", 21, ChatColor.DARK_GRAY, Material.FLINT),
			GUNPOWDER = new BasicDye(DYE_GUNPOWDER.get(), "GUNPOWDER", 22, ChatColor.GRAY, Material.SULPHUR),
			CYAN = new BasicDye(DYE_CYAN.get(), "CYAN", 23, ChatColor.DARK_AQUA, Material.INK_SACK, 6),
			PURPLE = new BasicDye(DYE_PURPLE.get(), "PURPLE", 24, ChatColor.DARK_PURPLE, Material.INK_SACK, 5),
			BLUE = new BasicDye(DYE_BLUE.get(), "BLUE", 25, ChatColor.DARK_BLUE, Material.INK_SACK, 4),
			BROWN = new BasicDye(DYE_BROWN.get(), "BROWN", 26, ChatColor.DARK_RED, Material.INK_SACK, 3),
			GREEN = new BasicDye(DYE_GREEN.get(), "GREEN", 27, ChatColor.DARK_GREEN, Material.INK_SACK, 2),
			BRICK = new BasicDye(Lang.DYE_BRICK.get(), "BRICK", 28, ChatColor.RED, Material.BRICK), // new
			BLACK = new BasicDye(DYE_BLACK.get(), "BLACK", 29, ChatColor.DARK_GRAY, Material.INK_SACK, 0),
			...
			LIGHT_WOOD = new BasicDye(Lang.DYE_LIGHT_WOOD.get(), "LIGHT_WOOD", 34, ChatColor.RED, Material.WOOD, 2), // new
			MAROON = new BasicDye(DYE_MAROON.get(), "MAROON", 35, ChatColor.DARK_RED, Material.NETHER_STALK),
			...
			LIGHT_BLUE_TERRACOTTA = new BasicDye(Lang.DYE_LIGHT_BLUE_TERRACOTTA.get(), "LIGHT_BLUE_TERRACOTTA", 39, ChatColor.DARK_GRAY, Material.MYCEL), // new
			...
			BROWN_TERRACOTTA = new BasicDye(Lang.DYE_BROWN_TERRACOTTA.get(), "BROWN_TERRACOTTA", 48, ChatColor.DARK_GRAY, Material.DIRT, 2), // new
			...
			BLACK_TERRACOTTA = new BasicDye(Lang.DYE_BLACK_TERRACOTTA.get(), "BLACK_TERRACOTTA", 51, ChatColor.DARK_GRAY, Material.COAL, 1), // new
